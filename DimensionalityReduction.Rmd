...
fromPy = arrange(fromPy, Station, 'Opened Year', Year)
```

```{r}
fromPy %>%
  ggplot(aes(x=Year, y=HouseholdMedianIncome)) +
...
selectionRace=c("Station", "White alone","Black or African American alone", "Hispanic or Latino")
selectionRiderships=c("Station", "Weekday ridership")
selectionIncome=c("Station", "Household Median Income")

dataToClusterRace=fromPy[,selectionRace]
```

**b.** Set labels as row index

```{r rownames, eval=TRUE}
dataToClusterRace$Station=NULL
row.names(dataToClusterRace)=dataToClusterRace$Station
```


**c.** Decide if data needs to be transformed:
```{r boxplotS, eval=TRUE}
boxplot(dataToClusterRace,horizontal = T, las=2,cex.axis=0.4)
```
...
library(cluster)
dataToCluster_DM=daisy(x=dataToClusterStandard, metric = "gower")
```
...
aggregate(data=fromPy,
          HouseholdMedianIncome~pam,
          FUN=mean)
```
```{r clusAgnCheck, eval=TRUE}
aggregate(data=fromPy,
          HouseholdMedianIncome~agn,
          FUN=mean)
```

```{r clusDiaCheck, eval=TRUE}
aggregate(data=fromPy,
          HouseholdMedianIncome~dia,
          FUN=mean)
```
